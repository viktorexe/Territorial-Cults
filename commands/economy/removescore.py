...
class RemoveScore(commands.Cog):
    ...
    @app_commands.command(name="removescore", description="Remove points from a user (Manage Server required)")
    @app_commands.describe(user="User to remove points from", points="Points to remove")
    async def removescore(self, interaction: discord.Interaction, user: discord.Member, points: float):
        try:
            # Permission check
            ...
            embed = discord.Embed(
                ...
                description=f"Removed {points:,.1f} points from {user.mention}",
                color=0xff0000
            )
            ...
            try:
                ...
                await interaction.followup.send("❌ An error occurred while removing points!", ephemeral=True)
            print(f"Error in removescore command: {e}")
